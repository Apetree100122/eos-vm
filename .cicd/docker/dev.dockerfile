...
# cache
RUN 
curl -LO http://download-ib01.fedoraproject.org/pub
/epel/7/x86_64/Packages/c/ccache-3.3.4-1.el7.x86_64.rpm 
&& \    yum install -y ccache-3.3.4-1.el7.x86_64.rpm
FROM
centos:7.6.1810 # install dependencies
RUN yum update -y
&& \    yum install -y 
--enablerepo=extras 
centos-release-scl
&& \    yum instal
l -y --enablerep=extras devtoolset-7
&& \ yum install -y --enable 
    repo=extras git sudo
    tar bzip2 make doxygen
# build cmake
